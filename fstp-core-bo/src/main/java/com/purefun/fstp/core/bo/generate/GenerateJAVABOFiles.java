...
//		String targetDirectory = directory.getPath() + "\\protofile\\";
		...
		if (flist == null || flist.length == 0) {
			...
			for(File f:flist) {
				String strCmd = "D:/software/protobuf-3.5.1/src/protoc.exe -I=./" + protofileDirectory + " --java_out=./src/main/java/ ./" + protofileDirectory + f.getName();  
				try {
...
//		String bodir = "resource/com/purefun/fstp/core/bo/";
		String bodir = args[0];
		GenerateJAVABOFiles writer = new GenerateJAVABOFiles(bodir);		
